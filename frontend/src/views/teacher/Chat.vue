<template>
  <div style="display:flex;height:calc(100vh - 120px);gap:16px">
    <div style="width:200px;background:white;border-radius:8px;padding:8px;overflow-y:auto">
      <h4>学生列表</h4>
      <div v-for="s in contacts" :key="s.id" @click="selectContact(s)"
           :style="{padding:'8px',cursor:'pointer',borderRadius:'4px',margin:'4px 0',background:selectedId===s.id?'#e6f7e6':'#f5f5f5'}">
        {{ s.username }}
      </div>
    </div>
    <div style="flex:1;background:white;border-radius:8px;display:flex;flex-direction:column">
      <div style="padding:12px;border-bottom:1px solid #eee"><b>{{ selectedName || '选择学生' }}</b></div>
...
onMounted(async () => {
  const r = await request.get('/teacher/chat/contacts'); contacts.value = r.data || []
})
async function selectContact(s) {
  selectedId.value = s.id; selectedName.value = s.username
  const r = await request.get('/teacher/chat/messages/' + s.id + '?teacherId=' + userId)
